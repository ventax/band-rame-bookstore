...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-purple-50 via-gray-50 to-pink-50 py-8 pb-24 md:pb-8">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex items-center justify-center max-w-xs sm:max-w-sm mx-auto">
            ...
            <div class="flex flex-col items-center">
                <div class="w-10 h-10 rounded-full bg-purple-600 text-white flex items-center justify-center shadow-md font-bold">
                    2
                </div>
            ...
            <div class="flex flex-col items-center">
                <div class="w-10 h-10 rounded-full bg-gray-200 text-gray-400 flex items-center justify-center shadow-sm font-bold">
                    3
                </div>
        ...
        <form action="{{ route('checkout.process') }}" method="POST">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-5">
                            <p class="text-gray-500 text-sm mb-4 leading-relaxed">
                                Setelah klik <strong class="text-gray-700">Buat Pesanan & Bayar</strong>, Anda akan diarahkan ke halaman pembayaran Midtrans. Pilih metode yang Anda inginkan:
                            </p>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                <div class="flex flex-col items-center gap-1.5 bg-blue-50 rounded-xl p-3 text-center">
                                    ...
                                    <span class="text-xs font-semibold text-blue-700 leading-tight">Transfer Bank / VA</span>
                                </div>
                                <div class="flex flex-col items-center gap-1.5 bg-red-50 rounded-xl p-3 text-center">
                                    ...
                                    <span class="text-xs font-semibold text-red-600 leading-tight">Kartu Kredit / Debit</span>
                                </div>
                                <div class="flex flex-col items-center gap-1.5 bg-green-50 rounded-xl p-3 text-center">
                                    ...
                                    <span class="text-xs font-semibold text-green-700 leading-tight">GoPay / OVO / DANA</span>
                                </div>
                                <div class="flex flex-col items-center gap-1.5 bg-yellow-50 rounded-xl p-3 text-center">
                                    ...
                                    <span class="text-xs font-semibold text-yellow-700 leading-tight">Indomaret / Alfamart</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                        <div class="flex items-center justify-between mb-3">
                            <h3 class="font-bold text-gray-900 flex items-center gap-2 text-sm">
                                <span class="w-7 h-7 rounded-full bg-purple-100 flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-map-marker-alt text-purple-600 text-xs"></i>
                                </span>
                                Alamat Pengiriman
                            </h3>
                            <a href="{{ route('checkout.address') }}" class="text-xs text-purple-600 hover:text-purple-800 font-semibold flex items-center gap-1">
                        ...
                        <div class="bg-purple-50 rounded-xl p-4 text-sm border border-purple-100">
                            ...
                            @else
                                ...
                                <p class="text-gray-600">{{ request('city') }}, {{ request('province') }} {{ request('postal_code') }}</p>
                            @endif
                        </div>
                    </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 lg:sticky lg:top-24 overflow-hidden">
                            ...
                            @foreach ($cartItems as $item)
                                <div class="flex gap-3 items-start">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-xs font-semibold text-gray-800 truncate">{{ $item->book->title }}</p>
                                        <p class="text-xs text-gray-400">{{ $item->quantity }}x • Rp {{ number_format($item->book->discounted_price, 0, ',', '.') }}</p>
                                    </div>
                                    <span class="text-xs font-bold text-gray-700 whitespace-nowrap">
                                        Rp {{ number_format($item->quantity * $item->book->discounted_price, 0, ',', '.') }}
                                    </span>
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="px-5 py-4 border-t border-gray-100 space-y-2">
                            ...
                            <div class="border-t border-dashed border-gray-200 pt-3 flex justify-between items-center">
                                ...
                                <span class="font-bold text-lg text-purple-600">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
                            </div>
                        </div>
...
<div class="fixed bottom-0 left-0 right-0 z-40 lg:hidden bg-white border-t border-gray-200 px-4 py-3 shadow-2xl">
    <div class="flex items-center justify-between mb-1.5">
        <span class="text-xs text-gray-500">Total Bayar</span>
        <span class="font-bold text-purple-600">Rp {{ number_format($grandTotal, 0, ',', '.') }}</span>
    </div>
    <button form="checkout-form-mobile" type="submit" onclick="document.querySelector('form').submit()"
        class="w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold py-3 rounded-xl text-sm flex items-center justify-center gap-2 shadow-lg">
        <i class="fas fa-lock text-xs"></i> Buat Pesanan & Bayar
    </button>
</div>
@endsection
